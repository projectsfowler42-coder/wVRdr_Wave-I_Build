...
import { ACTIVE_CONTAINERS, containerLabel, instrumentDisplayLabel } from "@/lib/containerModel";
...
function buildEmptyForm(container: ActiveContainerClass = DEFAULT_CONTAINER) {
  return {
    container,
    ticker: getBucketScopedInstruments(container)[0]?.ticker ?? "",
    shares: "",
    entryDate: "",
    entryPrice: "",
    notes: "",
...
export default function AddHoldingForm({ holdings, onHoldingsChange }: AddHoldingFormProps) {
  ...
  function handleSubmit(event: FormEvent) {
    ...
    if (form.container === "MINT") return setError("|M| is staging cash, not ticker MINT ETF. Add securities under [B] or [G].");
    ...
    <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-[11px] text-muted-foreground">Bridge mode excludes |W|. MINT ETF is a Blue instrument; |M| is staging cash.</div>
        </div>
        <button onClick={() => { setOpen(false); reset(); }} className="text-muted-foreground hover:text-foreground"><X size={16} /></button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Field label="Instrument"><select value={form.ticker} onChange={(event) => set("ticker", event.target.value)} className={selectCls} disabled={scopedInstruments.length === 0}>{scopedInstruments.length === 0 ? <option value="">No security in |M| wallet</option> : scopedInstruments.map((instrument) => <option key={instrument.ticker} value={instrument.ticker}>{instrumentDisplayLabel(instrument.ticker)} — {instrument.name}</option>)}</select></Field>
        ...
        {selectedInstrument ? <div className="rounded-xl border border-border/70 bg-background/60 p-3 text-xs text-muted-foreground"><div className="font-medium text-foreground">Selected instrument</div><div className="mt-1">{instrumentDisplayLabel(selectedInstrument.ticker)} · {selectedInstrument.subtype} · {selectedInstrument.payoutFrequency ?? "varies"}</div></div> : null}
        {error ? <div className="text-xs text-destructive">{error}</div> : null}
        <div className="flex items-center gap-2"><button type="submit" className="rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90">Save holding</button><button type="button" onClick={() => { setOpen(false); reset(); }} className="rounded-xl px-4 py-2 text-sm text-muted-foreground transition-colors hover:text-foreground">Cancel</button></div>
      </form>
    </div>
  );
}
