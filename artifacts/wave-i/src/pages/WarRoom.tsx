...
function PositionTelemetry({ contexts }: { contexts: ReturnType<typeof deriveHoldingContext>[] }) {
  ...
  if (top.length === 0) {
    return <div className="rounded-lg border border-border bg-card p-4 text-xs text-muted-foreground">No holdings yet. Add positions to start building personal capital telemetry.</div>;
  }
        ...
        <div key={ctx.holdingId} className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="text-right">
              ...
              <div className="text-[10px] text-muted-foreground">{ctx.snapshotAt ? "Refreshed" : "Awaiting refresh"}</div>
            </div>
          </div>
...
export default function WarRoom() {
  ...
  const holdingContexts = useMemo(() => holdings.map((holding) => deriveHoldingContext({ holding, quote: quotesByTicker[holding.ticker] })), [holdings, quotesByTicker]);
  ...
  async function handleHarvest() {
    ...
    try {
      const refreshed = uniqueTickers.length > 0 ? await refreshQuotes(uniqueTickers) : { quotes: {}, failed: [] as string[] };
      ...
      const now = Date.now();
      setLastUpdated(now);
      setHarvestReport({ updated: Object.keys(refreshed.quotes).length, skippedDuplicate, failed: refreshed.failed.length, finishedAt: new Date(now).toISOString() });
      setHarvestState("completed");
    } catch {
      const now = Date.now();
      setLastUpdated(now);
      setHarvestReport({ updated: 0, skippedDuplicate, failed: uniqueTickers.length || 1, finishedAt: new Date(now).toISOString() });
      setHarvestState("completed");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <Header tab={tab} onTabChange={setTab} onHarvest={handleHarvest} harvestState={harvestState} harvestSummary={harvestSummary} lastUpdated={lastUpdated} holdingsCount={holdings.length} />

      {tab === "warroom" && (
        <main className="flex-1 p-4 md:p-6 space-y-5 max-w-7xl mx-auto w-full">
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            <TelemetryCard label="Total capital" value={fmtDollar(deployed + reserve)} detail="Capital surface anchored to local holdings and snapshots" />
            <TelemetryCard label="Deployed" value={fmtDollar(deployed)} detail="Current marked position value or cost-basis fallback" />
            <TelemetryCard label="Unrealized" value={fmtDollar(capitalSummary.unrealizedGL)} detail={fmtPct(capitalSummary.unrealizedGLPct)} />
            <TelemetryCard label="Cash / reserve" value={fmtDollar(reserve)} detail="Treasury layer remains manual-first" />
            <TelemetryCard label="Transfers" value={fmtDollar(transfersInFlight)} detail="Pending transfer tracker follows next" />
            <TelemetryCard label="Refresh state" value={lastUpdated ? new Date(lastUpdated).toLocaleTimeString() : "Manual"} detail={harvestSummary ?? "One press = one refresh pass"} />
          </section>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[10px] text-muted-foreground">Capital first · market wrapped around holdings</span>
            </div>
            <PositionTelemetry contexts={holdingContexts} />
          </section>

          <section className="rounded-lg border border-border bg-card p-4">
            <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Block-4B shell state</div>
            <div className="mt-2 text-sm text-foreground">The recovery shell has been replaced with a capital-first surface. Quotes are supporting context only and the page must stay useful even when refresh is blank or partial.</div>
            <div className="mt-2 text-xs text-muted-foreground">Next home-page expansions belong to treasury, transfers, dates, and decision memory — not more quote-board theater.</div>
          </section>

          <section>
            <div className="flex items-center justify-between gap-3 mb-3">
              <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Supporting market context</h2>
              <span className="text-[10px] text-muted-foreground">Secondary to capital, still manually refreshed</span>
            </div>
            <BucketQuoteBoard />
          </section>

          <footer className="py-4 text-center text-[10px] text-muted-foreground border-t border-border/40">Wave-I | capital-first shell active | quotes secondary | frontend-only operator surface</footer>
        </main>
      )}
      ...
      {tab === "portfolio" && (
        <main className="flex-1 p-4 md:p-6 space-y-4 max-w-7xl mx-auto w-full">
          ...
          <footer className="py-4 text-center text-[10px] text-muted-foreground border-t border-border/40">Wave-I | portfolio data stored locally | M/W/B/G container semantics active</footer>
        </main>
      )}
    </div>
  );
