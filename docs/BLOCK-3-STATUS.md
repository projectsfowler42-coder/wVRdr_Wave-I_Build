...
Block-3 is no longer only quarantined reference material in this repository.

Selected Block-3 elements are now operative inside the frontend-only Wave-I runtime:
...
### 1. Quarantine reference
- `docs/BLOCK-3-QUARANTINE.md`
- purpose: preserve non-promoted Block-3 visual concept work
- authority: reference-only except for explicitly re-promoted elements
...
This file makes the repo state explicit so Block-3 is not mistaken for either:
- missing entirely
- fully complete already
